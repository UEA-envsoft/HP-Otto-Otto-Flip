...
class CommandExecutor:
    ...
    async def exec_command(self, command):
        ...
        try:
            rc = __import__("rc")
            exec_globals = {
                'print': lambda x: self.ble_print_async(f"b:{x}"),
                'rc': rc,
...
def on_rx(executor, key):
    ...
    def callback(buffer):
        ...
        if buffer.decode("utf-8")[0] == "@":
            ...
            rc = __import__("rc")
            rc.remote_control(code[1:], executor.ble_print, executor.exec_running)
            return
